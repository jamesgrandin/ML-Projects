...
for line in df[columnName]:
    line = str(line)  # type casts line as string

    line = re.sub(r"<.*?>", "", line)  # quick removal of HTML tags
    line = re.sub("[^a-zA-Z@#]", " ", line)  # strip out all non-alpha text
    line = line.strip().lower()  # convert all text to lowercase
    line = re.sub("n t ", " not ", line)  # change orphan n't into not

    df[columnName].iloc[i] = line
    if (i % 500 == 0):
        print("Cleaning review number", i, "out of", len(df[columnName]))
    i += 1

print(df[columnName][0:20])  # print first 20 lines of cleaned text
...
import numpy as np  # increases efficiency of matrix operations
 # reads in data files of mixed data types
...
df
X_train, X_test, y_train, y_test = train_test_split(df[columnName], df[sentiment], test_size=0.2)
...
def clean_my_text(text):
    ...
    text = re.sub("n t ", " not ", line)# result from cleaning possessives
